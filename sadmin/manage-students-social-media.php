...
.page-title { color: var(--dark-blue); font-weight: 700; margin-bottom: 0; }
.filter-container {
    background: var(--white);
    border-radius: 12px;
    padding: 20px;
...
.student-name { font-weight: 700; font-size: 1.2rem; margin-bottom: 5px; }
.student-id { font-size: 0.9rem; opacity: 0.9; }
.student-body { padding: 20px; }
.student-details { margin-bottom: 15px; }
.detail-item { display: flex; justify-content: space-between; margin-bottom: 8px; padding-bottom: 8px; border-bottom: 1px solid #f0f0f0; }
...
.social-link:hover { background: var(--primary-blue); color: white; transform: scale(1.1); }
...
.no-students i { font-size: 3rem; color: #ccc; margin-bottom: 15px; }
.no-students p { color: var(--text-light); font-size: 1.1rem; }
...
.status-disabled { background: #FFEBEE; color: #D32F2F; }

@media (max-width: 768px) { .students-grid { grid-template-columns: 1fr; } }
</style>
</head>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo '    <div class="student-details"></div>';

        // Social links
        $socials = ['blog'=>'globe', 'facebook'=>'facebook', 'linkedin'=>'linkedin', 'github'=>'github',
                    'whatsapp'=>'whatsapp', 'twitter'=>'twitter-x', 'instagram'=>'instagram',
                    'youtube'=>'youtube', 'tiktok'=>'tiktok', 'telegram'=>'telegram', 
                    'discord'=>'discord', 'reddit'=>'reddit', 'snapchat'=>'snapchat', 'pinterest'=>'pinterest',
                    'spotify'=>'spotify', 'dribbble'=>'dribbble', 'behance'=>'behance'];

        echo '<div class="social-links">';
        foreach ($socials as $key => $icon) {
            if (!empty($row[$key])) {
                echo '<a href="' . htmlspecialchars($row[$key]) . '" target="_blank" class="social-link">';
